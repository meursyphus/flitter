  ...
  test("renders every Charts/* story and produces non-empty output", async ({
    ...
    for (const story of stories) {
      for (const renderer of renderers) {
        ...
        expect(surfaceInfo, `${story.id} did not expose a render surface (${renderer})`).not.toBeNull();
        ...
        if (surfaceInfo?.kind === "svg") {
          ...
          expect(surfaceInfo.drawNodes, `${story.id} svg draw nodes (${renderer})`).toBeGreaterThan(3);
        }
