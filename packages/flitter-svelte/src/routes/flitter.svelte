<script lang="ts">
	import { Image, Colors, ColoredBox, Center, ObjectFit, ObjectPosition, type Widget } from '@meursyphus/flitter';
	import SvelteWidget from '../lib/Widget.svelte';
	import profile from './profile.png';

	const createImageCase = (width?: number, height?: number, objectFit: ObjectFit = ObjectFit.none) => {
		return Center({
			child: ColoredBox({
				color: Colors.white,
				child: Image({
	...
	const objectFits: ObjectFit[] = [ObjectFit.contain, ObjectFit.cover, ObjectFit.fill, ObjectFit.none, ObjectFit.scaleDown];
