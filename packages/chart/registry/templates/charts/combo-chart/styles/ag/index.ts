import type { ComboChartCustom } from "@headless/combo-chart/types";
import {
  Container,
  Stack,
  Alignment,
...
const agCustom: Partial<ComboChartCustom<ComboChartConfig>> = {
  ...
  bar: ({ value, label, legend, datasetIndex, yAxisId }, ctx) => {
    ...
    const color =
      ctx.config.colors.fills[
        datasetIndex % ctx.config.colors.fills.length
      ];
  ...
  line: ({ values, legend, datasetIndex, yAxisId, points }, ctx) => {
    ...
    const color =
      ctx.config.colors.strokes[
        datasetIndex % ctx.config.colors.strokes.length
      ];
