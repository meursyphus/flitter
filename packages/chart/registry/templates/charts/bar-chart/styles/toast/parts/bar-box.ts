...
function computeTooltipLayout({
  ...
  barBoxGlobal: { x: number; y: number };
  barBoxSize: { width: number; height: number };
  ...
  const localX = barBoxGlobal.x - plotGlobal.x;
  const localY = barBoxGlobal.y - plotGlobal.y;
  const space: SpaceAround = {
    right: chartWidth - (localX + barBoxSize.width),
    left: localX,
    top: localY,
    bottom: chartHeight - (localY + barBoxSize.height),
  };
...
class _TooltipPositioner extends StatefulWidget {
  ...
  constructor({
    ...
    super();
    this.barBoxWidget = barBoxWidget;
...
class _TooltipPositionerState extends State<_TooltipPositioner> {
  ...
  private computeLayout() {
    ...
    const barBoxGlobal = renderObject.localToGlobal();
    const barBoxSize = renderObject.size;
  ...
  override build() {
    ...
    return Tooltip({
      ...
      tooltip: isHovered
        ...
        : SizedBox.shrink(),
      child: this.widget.barBoxWidget,
    });
  }
}
...
export function toastBarBox(
  ...
  const barBoxWidget = AnimatedFractionallySizedBox({
