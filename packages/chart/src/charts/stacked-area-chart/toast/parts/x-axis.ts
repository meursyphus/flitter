...
class _AnimatedXAxisState extends State<_AnimatedXAxis> {
  ...
  override build() {
    ...
    // Stacked area chart always has label x-axis (category axis)
...
export function createToastXAxis(vc: ToastStackedAreaChartConfig) {
  return function toastXAxis(
    ...[{ line, labels, tick }]: Parameters<StackedAreaChartCustom["xAxis"]>
  ): Widget {
    return new _AnimatedXAxis({ line, labels, tick, config: vc });
  };
}
