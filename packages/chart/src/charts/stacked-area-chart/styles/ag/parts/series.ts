...
// --- Multi-series tooltip content (AG style: white bg with border) ---
...
function agMultiSeriesTooltipContent({
  label,
  items,
  config,
}: {
  label: string;
  items: SeriesItem[];
  config: AgStackedAreaChartConfig;
}): Widget {
    ...
    if (this.hovered && points.length > 0) {
      ...
      const tooltipWidget = ZIndex({
        ...
        child: Padding({
          ...
          child: agMultiSeriesTooltipContent({
            label,
            items: points.map((p) => ({
              legend: p.legend,
              color: p.color,
...
export function agSeries(
  ...
  const { tooltip, colors } = ctx.config;
