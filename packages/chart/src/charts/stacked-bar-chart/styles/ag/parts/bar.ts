...
class _HoverableBarState extends State<_HoverableBar> {
  ...
  override build() {
    const { color, strokeColor, gap, tooltip } = this.widget;
    ...
    const decoration = this.hovered
      ? new BoxDecoration({
          ...
          boxShadow: [
            new BoxShadow({ color: "rgba(0,0,0,0.2)", blurRadius: 6 }),
          ],
        })
      : new BoxDecoration({
          color,
...
export function agBar(
  ...
  return new _HoverableBar({
    ...
    strokeColor,
    gap: bar.gap,
    tooltip: agTooltipContent({ label, legend, color, value, config: context.config }),
