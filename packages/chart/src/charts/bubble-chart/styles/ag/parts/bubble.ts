...
// --- Bubble tooltip content (AG style: white bg, border) ---
...
class _HoverableBubbleState extends State<_HoverableBubble> {
  ...
  override build(): Widget {
    const { bubbleWidget, legend, fillColor, strokeColor, radius, dataX, dataY, config } = this.widget;
    ...
    if (this.hovered) {
      // Full opacity bubble + stroke border on hover
      children.push(
        Positioned({
          key: "__highlight__",
          top: 0, left: 0, bottom: 0, right: 0,
          child: Container({
            ...
            decoration: new BoxDecoration({
              color: fillColor,
              shape: "circle",
              border: Border.all({ color: strokeColor, width: 2, strokeAlign: 1 }),
              boxShadow: [
                new BoxShadow({ color: "rgba(0,0,0,0.2)", blurRadius: 8 }),
              ],
            }),
          }),
        }),
