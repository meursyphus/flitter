...
const HEADLESS_SHIMS = {
  ...
  "candlestick-chart": {
    ...
    aliases: {
      ...
      GetTicksFn: "CandlestickChartGetTicksFn",
    },
  },
...
function rewriteHeadlessTypeSpecifiers(specifiers, chartName) {
  ...
  return specifiers
    ...
    .replace(
      /\bGetTicksFn\b/gu,
      aliases.GetTicksFn ? `${aliases.GetTicksFn} as GetTicksFn` : "GetTicksFn",
    );
}
...
export async function renderTemplateFile({
  ...
  content = content
    ...
    .replace(
      /(['"])(?:\.\.\/)+shared\/(bar-like|line-like|pie-like|point-like)(?:\/([^'"]+))?\1/gu,
      (_, quote, helperName, subpath) => {
        const styleBase =
          item.style === "ag"
            ? agBaseOutputDir
            : item.style === "toast"
              ...
              : toastBaseOutputDir;
        const destination =
          subpath == null || subpath === "index"
            ? `${styleBase}/${helperName}/index.ts`
            : `${styleBase}/${helperName}/${subpath}`;
        return `${quote}${relativeTo(destination)}${quote}`;
      },
    )
    .replace(
  ...
  if (item.style != null) {
    ...
    } else if (new RegExp(`/styles/${item.style}/[^/]+$`, "u").test(file.source)) {
