...
export function html(
  ...
  const treatedValues = values.map((value) =>
...
export const Meta = html`
  ...
  <link
    rel="icon"
    href="https://docs.astro.build/favicon.ico"
    type="image/x-icon"
  />
  <link
    rel="icon"
    href="https://docs.astro.build/favicon.svg"
    type="image/svg+xml"
  />
`;
...
export const LocaleDetails = (
  ...
  const missingFiles = status.filter(
    (file) =>
      file.localizations.find((localization) => localization.lang === lang)
        ?.status === "missing",
  );
  const outdatedFiles = status.filter((file) => {
    ...
    if (file.type === "dictionary")
      return "missingKeys" in localization
    ...
    <details class="progress-details">
      <summary>
        ...
        <span class="progress-summary">
          ${doneLength.toString()} done, ${outdatedFiles.length.toString()}
          outdated, ${missingFiles.length.toString()} missing
        </span>
        <br />
        ${ProgressBar(status.length, outdatedFiles.length, missingFiles.length)}
      </summary>
      ${outdatedFiles.length > 0
        ? OutdatedFiles(outdatedFiles, lang, lunaria)
        : ""}
            ...
            <ul>
              ${missingFiles.map((file) => {
                ...
                return html`
                  <li>
                    ${Link(
                      links.source(file.source.path),
                      collapsePath(file.source.path),
                    )}
                    ${CreateFileLink(
          ...
          <li>
            ${isMissingKeys
              ? html`
                  <details>
                    <summary>
                      ${ContentDetailsLinks(file, lang, lunaria)}
                    </summary>
                    <h4>Missing keys</h4>
                    <ul>
                      ${localization.missingKeys.map(
                        (key) => html`<li>${key}</li>`,
                      )}
                    </ul>
                  </details>
...
export const StatusByFile = (
  ...
  return html`
    ...
    <table class="status-by-file">
      <thead>
        <tr>
          ${["File", ...locales.map(({ lang }) => lang)].map(
            (col) => html`<th>${col}</th>`,
          )}
        </tr>
      </thead>
...
export const ContentDetailsLinks = (
  ...
  return html`
    ${Link(
      links.source(fileStatus.source.path),
      collapsePath(fileStatus.source.path),
    )}
    (${Link(
      links.source(localization.path),
...
export const ProgressBar = (
  ...
  return html`
    <div class="progress-bar" aria-hidden="true">
      ${getBlocks(doneSize, "up-to-date")}
      ${getBlocks(outdatedSize, "outdated")}
...
export const TitleParagraph = html`
  <p>
    ...
    <a href="https://docs.astro.build/">docs.astro.build</a> into one of the
    languages listed below, you've come to the right place! This auto-updating
    page always lists all the content that could use your help right now.
  </p>
  <p>
