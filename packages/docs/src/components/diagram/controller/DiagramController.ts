...
class DiagramController {
  ...
  updateProject(project: Project) {
    ...
    const { relatedFieldIdsOnTableId, relatedFields } =
      this.resolveTableAndRefs({ tables, refs });
  ...
  private resolveTableAndRefs({
    ...
    const endpointToField = tables
      .map((table) =>
        table.fields.map((field) => ({ ...field, tableId: table.id })),
      )
      .flat()
      .map((field) =>
      ...
      .reduce(
        ...
        {} as Record<
          number,
          { fieldId: number; relation: Relation; tableId: number }
        >,
      );

    const relatedFieldIdsOnTableId: RelatedFieldByTableId = tables.reduce(
      (acc, table) => {
        acc[table.id] = [];
        return acc;
      },
    ...
    refs.forEach((ref) => {
      ...
      relatedFieldIdsOnTableId[from.tableId].push({
        from: from.fieldId,
        to: to.fieldId,
      });
      relatedFieldIdsOnTableId[to.tableId].push({
        from: from.fieldId,
        to: to.fieldId,
      });
