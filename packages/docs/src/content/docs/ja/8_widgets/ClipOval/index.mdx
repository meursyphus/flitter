...
nav_title: ClipOval
title: "ClipOval"
description: "楕円形または円形で子要素をクリップするウィジェット"
---

## 概要

ClipOvalは、子要素を楕円形または円形でクリップするウィジェットです。プロフィール画像やアバターの円形表示、デザイン目的での楕円形状が必要な場合によく使用されます。

> 楕円形を使用して子要素をクリップするウィジェット。

Flutter参考: https://api.flutter.dev/flutter/widgets/ClipOval-class.html
...
- 円形のプロフィール画像やアバターを作成する場合
- 画像やコンテナを楕円形にクリップする場合
- ボタンやカードを円形や楕円形にする場合
- デザインで滑らかな曲線形状が必要な場合
- ロゴやアイコンを円形の背景に配置する場合
- 丸い状態インジケーターやバッジを作成する場合
...
クリップ領域を定義する関数。ウィジェットのサイズを受け取り、楕円が描画される矩形領域を返します。
- `Rect.fromLTWH(0, 0, size.width, size.height)`: 全体領域を楕円としてクリップ
- `Rect.fromLTWH(10, 10, size.width-20, size.height-20)`: マージンを持つ楕円としてクリップ

### clipped（オプション）
**型: boolean**（デフォルト: **true**）

クリップ効果を有効にするかどうかを指定します。
- `true`: クリップを適用
- `false`: クリップを無効化（子ウィジェットは通常通り表示）
...
- **クリップのパフォーマンス**: 複雑な形状のクリップはレンダリングパフォーマンスに影響する可能性があります
- **境界の処理**: クリップされた領域外のタッチイベントは検出されません
- **アニメーション**: アニメーションとクリップを組み合わせる場合はパフォーマンスを考慮してください
- **クリッパー関数**: クリッパー関数はウィジェットのサイズが変更されるたびに呼ばれるため、重い計算は避けてください
- **ネストしたクリップ**: 複数のクリップウィジェットをネストするとパフォーマンスが低下する可能性があります

## 関連ウィジェット

- **ClipRRect**: 角丸でクリップするウィジェット
- **ClipRect**: 矩形でクリップするウィジェット
- **ClipPath**: カスタムパスでクリップするウィジェット
- **Container**: decorationのshapeプロパティで円形形状を作成可能
- **CircleAvatar**: 円形アバター作成専用ウィジェット
