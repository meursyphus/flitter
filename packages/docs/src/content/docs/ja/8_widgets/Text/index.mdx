---
nav_group: Widgets
nav_group_order: 3
nav_title: Text
title: "Text"
description: "スタイル付きテキストを表示する基本ウィジェット"
---

## 概要

Textは、スタイル付きテキストを表示する基本ウィジェットです。

Textウィジェットは単一スタイルのテキストを表示します。文字列はレイアウト制約によって複数行に分かれるか、すべて同じ行に表示される可能性があります。styleプロパティを使用してテキストの外観をカスタマイズできます。複数のスタイルを使用する必要がある場合は、Text.richコンストラクタを使用するか、RichTextウィジェットを直接使用できます。

## 使用場面

- 画面上にテキストを表示する場合
- タイトル、本文、ラベルなどを表示する場合
- 単一スタイルのテキストが必要な場合
- テキストオーバーフローの処理が必要な場合
- テキストの配置が必要な場合

## 基本的な使用方法

```typescript
// Basic text
Text('Hello, Flitter!')

// Styled text
Text('Styled text', {
  style: TextStyle({
    fontSize: 24,
    fontWeight: 'bold',
    color: 'blue'
  })
})

// Multi-styled text (Text.rich)
Text.rich(
  TextSpan({
    text: 'Hello ',
    children: [
      TextSpan({
        text: 'Flitter',
        style: TextStyle({ color: 'blue', fontWeight: 'bold' })
      }),
      TextSpan({ text: '!' })
    ]
  })
)
```

## コンストラクタ

### Text(data, props?)

デフォルトコンストラクタは単一スタイルのテキストを作成します。

```typescript
Text('Basic text')
Text('Styled text', { style: TextStyle({ fontSize: 18 }) })
```

### Text.rich(textSpan, props?)

複数のスタイルを持つテキストを作成します。

```typescript
Text.rich(
  TextSpan({
    text: 'Base text ',
    style: TextStyle({ fontSize: 16 }),
    children: [
      TextSpan({
        text: 'emphasized text',
        style: TextStyle({ fontWeight: 'bold', color: 'red' })
      })
    ]
  })
)
```

## プロパティ

### style

**型: TextStyle | undefined**

テキストスタイルを定義します。nullの場合、デフォルトのテキストスタイルが使用されます。

```typescript
Text('Styled text', {
  style: TextStyle({
    fontSize: 20,
    fontWeight: 'bold',
    color: '#333333'
  })
})
```

### textAlign

**Value: TextAlign | undefined**

Defines the horizontal alignment of text.

Available values:
- `TextAlign.left`: Left aligned
- `TextAlign.right`: Right aligned
- `TextAlign.center`: Center aligned
- `TextAlign.justify`: Justified
- `TextAlign.start`: Start direction aligned (left in LTR)
- `TextAlign.end`: End direction aligned (right in LTR)

```typescript
Text('Center aligned text', {
  textAlign: TextAlign.center
})
```

### overflow

**Value: TextOverflow | undefined**

How to handle text overflow when it exceeds the space.

Available values:
- `TextOverflow.clip`: Clip the text (default)
- `TextOverflow.fade`: Fade out
- `TextOverflow.ellipsis`: Show ellipsis (...)
- `TextOverflow.visible`: Allow overflow

```typescript
Text('Very long text that will be displayed here...', {
  overflow: TextOverflow.ellipsis
})
```

### softWrap

**Value: boolean | undefined** (default: **true**)

Whether the text should break at soft line breaks.

```typescript
Text('Long text will automatically wrap to the next line', {
  softWrap: true  // Allow wrapping
})

Text('This text will only be displayed on one line', {
  softWrap: false  // No wrapping
})
```

### textDirection

**Value: TextDirection | undefined**

The directionality of the text.

Available values:
- `TextDirection.ltr`: Left to right
- `TextDirection.rtl`: Right to left

```typescript
Text('مرحبا بالعالم', {
  textDirection: TextDirection.rtl
})
```

### textWidthBasis

**Value: TextWidthBasis | undefined**

Defines how to measure the width of the rendered text.

Available values:
- `TextWidthBasis.parent`: Width of parent (default)
- `TextWidthBasis.longestLine`: Width of longest line

```typescript
Text('Multi-line\ntext', {
  textWidthBasis: TextWidthBasis.longestLine
})
```

## TextStyleプロパティ

TextStyleはテキストの視覚的プロパティを定義します。

### color

**Value: string | undefined**

The text color.

```typescript
TextStyle({ color: 'blue' })
TextStyle({ color: '#FF5733' })
TextStyle({ color: 'rgba(255, 0, 0, 0.5)' })
```

### fontSize

**Value: number | undefined**

The font size in pixels.

```typescript
TextStyle({ fontSize: 16 })  // 16px
TextStyle({ fontSize: 24 })  // 24px
```

### fontWeight

**Value: string | undefined**

The font weight.

Available values:
- `'normal'` or `'400'`
- `'bold'` or `'700'`
- `'100'` to `'900'` (in increments of 100)

```typescript
TextStyle({ fontWeight: 'bold' })
TextStyle({ fontWeight: '600' })
```

### fontStyle

**Value: FontStyle | undefined**

The font style.

Available values:
- `FontStyle.normal`: Normal
- `FontStyle.italic`: Italic

```typescript
TextStyle({ fontStyle: FontStyle.italic })
```

### fontFamily

**Value: string | undefined**

The font family to use.

```typescript
TextStyle({ fontFamily: 'Roboto' })
TextStyle({ fontFamily: 'Arial, sans-serif' })
```

### height

**Value: number | undefined**

The line height as a multiple of the font size.

```typescript
TextStyle({ 
  fontSize: 16,
  height: 1.5  // Line height is 24px (16 * 1.5)
})
```

### inherit

**Value: boolean** (default: **true**)

Whether to inherit the parent's text style.

```typescript
TextStyle({ 
  inherit: false,  // Ignore parent style
  fontSize: 20
})
```

## 実用的な例

### 例1: タイトルと本文

```typescript
const ArticleHeader = () => {
  return Column({
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text('From Flutter to Flitter', {
        style: TextStyle({
          fontSize: 28,
          fontWeight: 'bold',
          color: '#1a1a1a'
        })
      }),
      SizedBox({ height: 8 }),
      Text('A new way to build Flutter apps with JavaScript', {
        style: TextStyle({
          fontSize: 16,
          color: '#666666',
          height: 1.5
        })
      }),
      SizedBox({ height: 4 }),
      Text('January 10, 2024 • 5 min read', {
        style: TextStyle({
          fontSize: 14,
          color: '#999999'
        })
      })
    ]
  });
};
```

### 例2: 省略表示付きカード

```typescript
const ProductCard = ({ title, description, price }) => {
  return Container({
    padding: EdgeInsets.all(16),
    decoration: BoxDecoration({
      color: 'white',
      borderRadius: BorderRadius.circular(8),
      boxShadow: [
        BoxShadow({
          color: 'rgba(0, 0, 0, 0.1)',
          blurRadius: 4,
          offset: { x: 0, y: 2 }
        })
      ]
    }),
    child: Column({
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(title, {
          style: TextStyle({
            fontSize: 18,
            fontWeight: 'bold'
          }),
          overflow: TextOverflow.ellipsis
        }),
        SizedBox({ height: 8 }),
        Text(description, {
          style: TextStyle({
            fontSize: 14,
            color: '#666666',
            height: 1.4
          }),
          overflow: TextOverflow.ellipsis,
          softWrap: true
        }),
        SizedBox({ height: 12 }),
        Text(`$${price.toFixed(2)}`, {
          style: TextStyle({
            fontSize: 20,
            fontWeight: 'bold',
            color: '#FF5733'
          })
        })
      ]
    })
  });
};
```

### 例3: 混合スタイルテキスト

```typescript
const TermsText = () => {
  return Text.rich(
    TextSpan({
      text: 'By continuing, you agree to our ',
      style: TextStyle({ fontSize: 14, color: '#666' }),
      children: [
        TextSpan({
          text: 'Terms of Service',
          style: TextStyle({ 
            color: 'blue',
            fontWeight: 'bold'
          })
        }),
        TextSpan({ text: ' and ' }),
        TextSpan({
          text: 'Privacy Policy',
          style: TextStyle({ 
            color: 'blue',
            fontWeight: 'bold'
          })
        }),
        TextSpan({ text: '.' })
      ]
    })
  );
};
```

### 例4: チャットメッセージ

```typescript
const ChatMessage = ({ message, isMe, time }) => {
  return Container({
    margin: EdgeInsets.symmetric({ horizontal: 16, vertical: 4 }),
    alignment: isMe ? Alignment.centerRight : Alignment.centerLeft,
    child: Container({
      padding: EdgeInsets.symmetric({ horizontal: 12, vertical: 8 }),
      decoration: BoxDecoration({
        color: isMe ? '#007AFF' : '#E5E5EA',
        borderRadius: BorderRadius.circular(18)
      }),
      constraints: Constraints({ maxWidth: 250 }),
      child: Column({
        crossAxisAlignment: CrossAxisAlignment.end,
        children: [
          Text(message, {
            style: TextStyle({
              fontSize: 16,
              color: isMe ? 'white' : 'black'
            })
          }),
          SizedBox({ height: 2 }),
          Text(time, {
            style: TextStyle({
              fontSize: 12,
              color: isMe ? 'rgba(255, 255, 255, 0.7)' : '#999999'
            })
          })
        ]
      })
    })
  });
};
```

### 例5: 価格表示

```typescript
const PriceDisplay = ({ originalPrice, salePrice, discountRate }) => {
  return Row({
    crossAxisAlignment: CrossAxisAlignment.baseline,
    textBaseline: TextBaseline.alphabetic,
    children: [
      if (originalPrice !== salePrice) ...[
        Text(`$${originalPrice.toFixed(2)}`, {
          style: TextStyle({
            fontSize: 14,
            color: '#999999',
            decoration: 'line-through'
          })
        }),
        SizedBox({ width: 8 })
      ],
      Text(`$${salePrice.toFixed(2)}`, {
        style: TextStyle({
          fontSize: 24,
          fontWeight: 'bold',
          color: '#1a1a1a'
        })
      }),
      if (discountRate > 0) ...[
        SizedBox({ width: 8 }),
        Text(`${discountRate}% OFF`, {
          style: TextStyle({
            fontSize: 18,
            fontWeight: 'bold',
            color: '#FF5733'
          })
        })
      ]
    ]
  });
};
```

## 重要な注意事項

- overflowプロパティはsoftWrapがfalseまたはテキストが1行に制限されている場合に最も効果的です。
- fontFamilyを使用する場合、フォントがシステムにインストールされているか、アプリに含まれていることを確認してください。
- heightプロパティは行間を調整し、fontSizeの倍数として動作します。
- Text.richを使用する場合、ベーススタイルはすべてのTextSpanに継承されます（inheritがtrueの場合）。

## 関連ウィジェット

- **RichText**: より複雑なテキストスタイリングが必要な場合
- **TextField**: ユーザー入力用
- **SelectableText**: 選択可能なテキストが必要な場合
- **DefaultTextStyle**: サブツリーのデフォルトテキストスタイルを定義する場合