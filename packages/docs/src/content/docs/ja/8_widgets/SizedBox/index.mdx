...
nav_title: SizedBox
title: "SizedBox"
description: "指定されたサイズのシンプルなボックスウィジェット"
---

## 概要

SizedBoxは、特定の幅と高さを持つボックスを作成するシンプルなウィジェットです。

SizedBoxはサイズ設定のみに焦点を当てた単一目的のウィジェットで、Containerよりも軽量で効率的です。ウィジェット間のスペーシング作成、子ウィジェットのサイズ制約、条件付きレンダリングでの空白スペース作成などによく使用されます。内部的にはタイトな制約を持つConstrainedBoxを使用します。
