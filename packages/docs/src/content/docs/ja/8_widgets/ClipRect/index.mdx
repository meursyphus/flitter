...
nav_title: ClipRect
title: "ClipRect"
description: "矩形を使用して子要素をクリップするウィジェット"
---

## 概要

ClipRectは、子ウィジェットを矩形領域でクリップするウィジェットです。

このウィジェットはRectオブジェクトを返すクリッパーコールバック関数を受け取り、子ウィジェットを任意の矩形領域にクリップできます。内部的にはClipPathを使用してクリッピングを実行します。
