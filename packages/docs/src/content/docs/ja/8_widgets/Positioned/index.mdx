...
title: "Positioned"
description: "Stack内で子の位置を制御するウィジェット"
---

## 概要

Positionedは、Stackの子要素の位置を制御するウィジェットです。

Positionedでは、Stackの端からの距離を指定して子ウィジェットを絶対的に配置できます。CSSの`position: absolute`と似ており、top、bottom、left、rightプロパティを使用して位置を制御します。PositionedはStackウィジェットの直接の子である必要があります。
