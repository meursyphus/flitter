...
AspectRatio는 자식 위젯을 특정 종횡비(가로세로 비율)로 크기를 조정하려고 시도하는 위젯입니다.

이 위젯은 먼저 레이아웃 제약 조건에서 허용하는 최대 너비를 시도합니다. 위젯의 높이는 주어진 종횡비를 너비에 적용하여 결정되며, 이는 너비 대 높이의 비율로 표현됩니다.

예를 들어, aspectRatio가 2.0이면 너비가 높이의 2배가 됩니다 (16:9는 16/9 = 1.78로 표현).
