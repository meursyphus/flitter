...
function checkOverlap(from: Rect, to: Rect) {
  return !(
    to.right + 2 * RELATION_LENGTH < from.left ||
    from.right + 2 * RELATION_LENGTH < to.left
  );
}

function getDotsOnOverlapped({
  fromField,
  toField,
}: {
  fromField: Vertex;
...
function getDotsNotOverlapped({
  ...
  const from = new Offset({
    x:
      fromField.rect.right < toField.rect.left
        ? fromField.rect.right
        : fromField.rect.left,
    y: fromField.rect.center.y,
  });
  const to = new Offset({
