...
export const barChartApiPage: ApiPageData = {
  slug: ["api", "bar-chart"],
  title: "Bar Chart API",
  description:
    "Complete API reference for the Bar Chart, including data format, configuration options, custom parts, and context methods.",
  pageType: "api",
  parent: "api",
  dataFormat: {
  ...
  datasets: { legend: string; values: number[] }[];
    ...
    description:
      "Each dataset represents a series with a legend name and an array of values corresponding to the labels.",
  },
  agConfig: {
    sections: [
      {
        ...
        rows: [
          {
            ...
            type: "number",
            default: "1",
            description: "Gap between bars within a group in pixels.",
          },
          {
            property: "bar.cornerRadius",
            type: "number",
            default: "0",
            description: "Corner radius of bar rectangles in pixels.",
          },
        ],
      },
    ],
  ...
  customParts: [
    { element: "barGroup", args: "{ bars: { bar: Widget; value: number; datasetIndex: number }[]; index: number; label: string }", description: "Container for a group of bars at a single label position." },
    { element: "barBox", args: "{ bar: Widget; value: number; ratio: number; alignment: Alignment; index: number; label: string; legend: string; isHovered: boolean }", description: "Wrapper around each individual bar, providing layout and alignment." },
    ...
    bar: (args, context) => {
      ...
      return Container({
        decoration: new BoxDecoration({
          color: args.isHovered ? color : \`\${color}88\`,
