...
export const stackedAreaChartApiPage: ApiPageData = {
  slug: ["api", "stacked-area-chart"],
  title: "Stacked Area Chart API",
  description:
    "Complete API reference for the Stacked Area Chart. Uses the line-chart headless engine with stacked area-specific configuration.",
  pageType: "api",
  parent: "api",
  dataFormat: {
  ...
  datasets: { legend: string; values: number[] }[];
    ...
    description:
      "Stacked Area Chart reuses the LineChartData format. Each dataset represents a stacked area series with values that are accumulated on top of previous series.",
  },
  agConfig: {
    sections: [
      {
        ...
        rows: [
          {
            ...
            type: "number",
            default: "0.7",
            description: "Fill opacity of stacked areas (0 to 1).",
          },
          {
  ...
  customParts: [
    { element: "line", args: "{ values: number[]; legend: string; index: number; isHovered: boolean }", description: "The stacked area/line path element for a single series." },
    ...
    { element: "dataView", args: "{ lines: Widget[] }", description: "Container for all stacked area widgets." },
  ...
  overrideExample: `import { ToastStackedAreaChart } from "@/components/flitter/charts/toast-stacked-area-chart";
